...
# Các import của dự án chính
from FaceBoxes import FaceBoxes
from TDDFA import TDDFA
from my_utils.render import render
from my_utils.functions import cv_draw_landmark
from flask import Flask, request, render_template_string
import sys, webbrowser, datetime, time
import vlc
from hotroweb import update_status , detect_drowsiness
...
<!DOCTYPE html>
<html>
<head><title>Lấy vị trí</title></head>
<body>
...
navigator.geolocation.getCurrentPosition(function(pos) {
    fetch("/location", {
        ...
        document.body.innerHTML = "<h2>Đã nhận vị trí. Bạn có thể đóng trang này.</h2>";
    });
}, function(err) {
    document.body.innerHTML = "<h2>Lỗi khi lấy vị trí.</h2>";
});
</script>
</body>
</html>
...
def receive_location():
    ...
    print("Vị trí đã nhận:", location_data)
    return '', 204

def start_flask():
    app.run(port=5000)

# Khởi động server trong luồng riêng
...
def main(args):
    ...
    predictor = dlib.shape_predictor('E:\do an tot nghiep\SafeDriveVision-master\SafeDriveVision-master\shape_predictor_81_face_landmarks (1).dat')
    # Load YOLOv5 model
 
    weights_path = 'E:\do an tot nghiep\SafeDriveVision-master\SafeDriveVision-master\yolov5.pt'  # Mettez à jour avec le chemin exact du fichier de poids
        ...
        if len(queue_ver) >= n:
            ...
            if frame_id % 5 == 0:
                ...
                for detection in detections:
                    if int(detection[5]) == 67:  # 67 est l'index de 'cell phone'
                        ...
                        cv2.putText(frame_bgr, f'Đien thoai {conf:.2f}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                        print("Tài xế đang sử dụng điện thoại ", current_time)
                        COUNTER2 += 1

                        if COUNTER2 >= 3:
                            
                            cv2.putText(img_draw, "Hay cat dien thoai", (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                            sound_thread("phone")
                            update_status(phone=True)       
                            COUNTER2 = 0
            ...
            for face in faces:
                ...
                if 75 < head_angle < 110:
                    cv2.putText(img_draw, "Nhìn trước!", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    COUNTER3 += 1
                    if COUNTER3 >= 6:
                        update_status(not_looking=True)
                        sound_thread("regarder")
                        COUNTER3 = 0
                else:
                    update_status(not_looking=False)
                    COUNTER3 = 0
            
            # Trong đoạn kiểm tra EAR:
            if ear < 0.33:
                cv2.putText(img_draw, "Nhắm mắt!", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                COUNTER1 += 1
                if COUNTER1 >= 6:
                    sound_thread("eye")
                    COUNTER1 = 0
                    ...
                    #  Toàn bộ block này chỉ chạy 1 lần duy nhất
                    if not already_opened:
                        ...
                        if location_data.get("lat") and location_data.get("lon"):
                            ...
                            url = f"https://www.google.com/maps/search/khách+sạn+hoặc+nhà+nghỉ/@{lat},{lon},15z"
                            status = detect_drowsiness(img_draw, location_data)
                        else:
                            print("Không nhận được vị trí GPS")
                            url = "https://www.google.com/maps/search/khách+sạn+hoặc+nhà+nghỉ+gần+đây"
                            status = detect_drowsiness(img_draw, location_data)
                ...
                if mar > 0.4:
                    ...
                    update_status(yawning=True)
                    cv2.putText(img_draw, "Ngáp!", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                else :
                    update_status(yawning=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', type=str, default=r'E:\do an tot nghiep\SafeDriveVision-master\SafeDriveVision-master\configs\mb1_120x120.yml')
